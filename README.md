...
The unpredictability of sports betting, particularly in soccer matches, presents a significant challenge for bettors and sports analysts alike. Soccer matches are inherently complex, influenced by a variety of factors such as team dynamics, individual player performance, strategies, weather conditions, and unexpected events like referee decisions. These variables make predicting match outcomes an intricate task, often resulting in financial losses and frustration for bettors relying on simplistic or heuristic methods.

Our research seeks to address this challenge by leveraging machine learning (ML) to develop a sophisticated model capable of predicting soccer match outcomes with greater accuracy. The use of ML allows us to move beyond traditional approaches, such as win-loss records or expert intuition, by analyzing large datasets to identify patterns and correlations that may not be immediately apparent. By employing advanced predictive algorithms, we aim to create a tool that provides bettors with data-driven insights, improving decision-making and potentially transforming the sports betting landscape.

By combining supervised and unsupervised ML techniques, we aim to explore the strengths of various algorithms in addressing this problem. Our ultimate goal is to demonstrate the potential of machine learning in enhancing the accuracy and reliability of predictions in a domain characterized by uncertainty and complexity. This research has broader implications, showcasing how data-driven approaches can revolutionize traditional methods in sports analytics and beyond.
...
3) We implemented K-Means as our unsupervised learning method to explore natural groupings in soccer match data and gain insights into patterns without relying on labeled outcomes. Key features, such as team shooting metrics (team_b_shots_overall_l4_TSR, team_b_shots_average) and expected goals (predict_xg_overall_team_b), were standardized using StandardScaler to ensure optimal performance, as K-Means relies on Euclidean distances sensitive to feature scale. With three clusters representing Home Win, Draw, and Away Win, the model provided valuable exploratory insights by revealing relationships between features and match outcomes. Visualizations in 2D and 3D showed the spatial distribution of clusters compared to actual outcomes, highlighting trends in performance metrics. Although the clusters did not perfectly match the true labels due to the unsupervised nature of K-Means, the analysis helped uncover meaningful patterns, offering a complementary perspective to supervised models.

## RESULTS AND DISCUSSION

### Comparison 
First let's compare the supervised learning methods that we chose: Random Forest and Logistic Regression. Although we obtained similar accuracies (75%) on our dataset from both of these models, these models differ in their advantages and disadvantages. Firstly, Random Forest models are complicated and expensive, while Logistic Regression is simple. Related to this, Random Forest is also less interpretable, in that it is difficult to analyze how the features are contributing to the output. On the other hand, Logistic Regression has direct coefficients that explain the relationship between a feature and the target. Also, because Logistic Regression is simple, it is less prone to overfitting. Along with being simple, Logistic Regression combats overfitting with its L1/L2 regularization. Random Forest, especially with an inefficient set of hyperparameters, is prone to overfitting due to improper tuning of factors such as tree depth. 
...
![IMG 1](Supervised/Logistic%20Regression%20Visualizations/ConfusionMatrix.png)
![IMG 1](Supervised/Logistic%20Regression%20Visualizations/ImportanceByFeatureChart.png)
